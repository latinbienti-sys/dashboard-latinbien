...
# Use CORRECT bot names for navigation
# Bot 65: MENU_RECOMPRA ✓ 
# Bot 66: MENU_LC_APROBADA ✓
# Bot 64: search by 'MENU_REGISTRADO' (ilike will find it)
# Bot 63: search by 'MENU_NO_REGISTRADO' (ilike will find it)
...
def send(txt, goto=None, clear=False):
    ...
    if clear:
        ...
        # Usar ilike para encontrar el bot por nombre parcial
        target = Bot.search([('name', 'ilike', goto)], limit=1)
        if target:
...
try:
    ...
    if resp.json().get('result'):
        print("✅ Bot 62: con navegación por ilike CORREGIDA")
    else:
        print("❌ Bot 62 error")
