...
def update_text_match(bid, tm):
    resp = write_bot(bid, {'text_match': tm})
    return resp.get('result', False)

# ======== PASO 1: Restaurar text_match de sub-menús ========
# Los sub-menús NO deben interceptar mensajes por texto
# Solo se navega a ellos mediante conv.write({'active_bot_id': ...})
print("=== Restaurando text_match de sub-menús ===")
for bid, tm_val in [(65, '__MENU_RECOMPRA__'), (66, '__MENU_LC__'), 
                      (64, '__MENU_REGISTRADO__'), (63, '__MENU_NR__'),
                      (84, '__TRANSFERIR__'), (101, '__BUSCAR__')]:
    ok = update_text_match(bid, tm_val)
    print(f"  Bot {bid}: text_match={tm_val} {'✅' if ok else '❌'}")

# ======== PASO 2: Restaurar bot 61 (CATCHER) ========
# Pide la cedula y navega a bot 62 usando el patrón de bot 40
...
if write_bot(61, {'code': code61}).get('result'):
    print("\n✅ Bot 61: pide cedula + conv.write(active=62) + ret = env['...']")
else:
    print("\n❌ Bot 61 error")

# ======== PASO 3: Restaurar bot 62 (VALIDAR_CEDULA) con el código COMPLETO ========
# Usa el código de fix_bot62.py que tiene toda la lógica de validación
# PERO usando el patrón de bot 40 (conv.write + ret = env['...'])
...
try:
    ...
    if write_bot(62, {'code': code62}).get('result'):
        print("✅ Bot 62: código COMPLETO de validación con ret = env['...']")
    else:
        print("❌ Bot 62 error")
