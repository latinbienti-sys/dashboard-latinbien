...
code62 = '''try:
    ...
    goto_bot = False  # False = stay on current bot
    ...
    if texto == '6':
        ...
        send('Perfecto! He recopilado toda tu informacion con exito. Para garantizarte una asesoria completamente personalizada, te estoy transfiriendo en este instante con uno de nuestros asesores.')
    ...
    conv = mess_id.conversation_id
    msg = {'ttype': 'text', 'from_me': True, 'contact_id': conv.id, 'text': 'Error: ' + str(e)}
    try:
        conv.send_message_bus_release(msg, 'current', False)
    except:
        conv.send_message_bus_release(msg, 'done')
...
try:
    ...
    if resp.json().get('result'):
        print("✅ Bot 62: navegación corregida (ret devuelve el bot destino)")
    else:
